...
export default function EmailChangeVerification() {
  ...
  const [verificationType, setVerificationType] = useState('email-change-step1');
  ...
  useEffect(() => {
    if (router.isReady) {
      ...
      if (router.query.type === 'email-change-step1') {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log('Email change verification with:', { userId, code, verificationType });
      ...
      if (verificationType === 'email-change-step1') {
        ...
        if (response.data && response.data.success) {
          console.log('Step 1 verification successful, redirecting to new email page');
          router.push('/profile/new-email');
        } else {
          setError('Hmm, that code isn\'t right.');
        }
      }
      ...
      if (error.response && error.response.data) {
        setError(error.response.data.message || 'Hmm, that code isn\'t right.');
      } else {
        setError('Hmm, that code isn\'t right.');
      }
    } finally {
        ...
        <div className="fixed top-0 left-0 right-0 bg-[#FBF5DA] z-20">
          <div className="w-full max-w-[390px] mx-auto">
            <div className="flex flex-row items-center px-[20px] h-[65px] gap-[10px]" style={{ paddingTop: 'env(safe-area-inset-top, 20px)' }}>
              <Link href="/profile/change-email" className="relative z-50 w-10 h-10 flex items-center justify-center -ml-2">
                <ArrowLeftIcon />
              </Link>
          ...
          <form id="verifyForm" onSubmit={handleSubmit} className="w-full">
            <div className="flex flex-col items-center w-full mt-4">
              <div className="flex flex-col items-center w-full">
                ...
                {error && (
                  <div className="mt-4 w-full max-w-[300px] flex items-center justify-center">
                    <span className="font-medium text-sm" style={{ color: '#E95858' }}>{error}</span>
                  </div>
                )}
              </div>
            </div>
