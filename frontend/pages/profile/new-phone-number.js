...
export default function NewPhoneNumber() {
  ...
  const handleSavePhone = async () => {
    ...
    // Extract only digits for comparison and saving
    const rawPhoneNumber = newPhoneNumber.replace(/\D/g, '');
    ...
    if (rawPhoneNumber === user.phone?.replace(/\D/g, '')) {
      toast.error('Please enter a different phone number than your current one');
      return;
    }
    ...
    try {
      // Update phone number directly using updateProfile (save raw digits)
  ...
  return (
    <div className="min-h-screen bg-[#FBF5DA] font-['Nunito']">
      <Head>
        <title>New Phone Number - tenantli</title>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[#FBF5DA] z-10">
        <div className="w-full max-w-[390px] mx-auto px-[20px] pb-[12px]">
          <button
            ...
            disabled={isSubmitting || !newPhoneNumber.trim() || newPhoneNumber.replace(/\D/g, '').length !== 10}
